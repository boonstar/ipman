...
class Makeup_artist(object):
    ...
    def opencv_process(self, frame):
            
        
        frameWidth = frame.shape[1]
        frameHeight = frame.shape[0]
        
        self.net.setInput(cv.dnn.blobFromImage(frame, 1.0, (self.inWidth, self.inHeight), (127.5, 127.5, 127.5), swapRB=True, crop=False))
        ...
        if points[2] and points[3] and points[4]:
            ...
            if all(y > shoulder-10 for y in y_coords) and all(y < shoulder+10 for y in y_coords):
                cv.putText(frame, 'Punching straight', (10, 80), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                self.score += 1
            else:
                pass
        ...
        cv.putText(frame, 'Score: {}'.format(self.score), (10, 100), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
        # CG detection
        # ----------------
    ...
    def apply_makeup(self, img):
        ...
        return self.opencv_process(open_cv_image)
